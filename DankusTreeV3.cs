...
namespace DankusTreeV3
{
    class SnowFlake
    {
        ...
        public Point point;
        public int H = DankusTreeV3.rnd.Next(0,5), W = DankusTreeV3.rnd.Next(0, 5);
    }

    public partial class DankusTreeV3 : Form
    {
        ...
        public DankusTreeV3()
        {
            ...
            for (int i = 0; i < FCount; i++)
            {
                ...
                Flakes[i].point = new Point(rnd.Next(0, Width), rnd.Next(-Height,0));
            }
        }
        ...
        private void tmr_Snow_Tick(object sender, EventArgs e)
        {
            ...
            {
                Flakes[i].X += rnd.Next(-1, 2);
                Flakes[i].Y += Flakes[i].W*Flakes[i].H/5;

                if (Flakes[i].Y > Height) Flakes[i].Y = 0;
                if (Math.Abs(Flakes[i].X) > Width * 2) { Flakes[i].point = new Point(rnd.Next(0, Width), 0); }
